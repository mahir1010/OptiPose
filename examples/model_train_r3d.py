...
# Usage python model_train_r3d.py n_cm k n_heads
...
R3D_config = OptiPoseConfig('./example_configs/Rodent3D.yml')
...
dataset_title = "Rodent3D"
multi_heads = int(sys.argv[3])
n_kps = R3D_config.num_parts  # 10 parts now including SP1 and SP2
batch_size = 100
dataset = {'input': [], 'label': []}
vdataset = {'input': [], 'label': []}

model = optipose_postural_autoencoder(30, n_kps, numPCMs, num_sub_ck, multi_heads, output_dim=60)
# model.summary()
...
vdataset = {'input': [], 'label': []}
reader = csv.reader(
    open(f'./augmented_train_test/R3D/IJCV_{dataset_title}_dataset_t_attention_{30}_{num_samples}.csv'),
    delimiter='|')

# Test file will still contain 6500 samples
reader1 = csv.reader(
    open(f'./augmented_train_test/R3D/IJCV_{dataset_title}_dataset_t_attention_{30}_{num_samples}_test.csv'),
    delimiter='|')
...
checkpoint_path = f"./Model_weights/R3D/optipose-{numPCMs}-{num_sub_ck}-{dataset_title}-{multi_heads}" + "/closs-{epoch:04d}.ckpt"
